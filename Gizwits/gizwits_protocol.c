...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempcontrol)
    {
        info->event[info->num] = EVENT_tempcontrol;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempcontrol,(uint8_t *)&issuedData[byteFieldOffset],tempcontrol_LEN);
        byteFieldOffset += tempcontrol_LEN;
        dataPoints->valuetempcontrol = gizX2Y(tempcontrol_RATIO,  tempcontrol_ADDITION, elongateP0FormTmp.devStatus.valuetempcontrol); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetempcontrol != cur->valuetempcontrol)
    {
        GIZWITS_LOG("valuetempcontrol Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempcontrol = 1;
        ret = 1;
    }
    ...
    if(last->valueHR != cur->valueHR)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHR Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagHR = 1;
            ret = 1;
        }
    }
    if(last->valueSp02 != cur->valueSp02)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSp02 Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagSp02 = 1;
            ret = 1;
        }
    }
    if(last->valuelatitude != cur->valuelatitude)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuelatitude Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaglatitude = 1;
            ret = 1;
        }
    }
    if(last->valuelongitude != cur->valuelongitude)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuelongitude Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaglongitude = 1;
            ret = 1;
        }
    }
    if(last->valueppm != cur->valueppm)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueppm Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagppm = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagHR)
    {
        devStatusTmp.valueHR = gizY2X(HR_RATIO,  HR_ADDITION, dataPoints->valueHR); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHR,HR_LEN);
        byteFieldOffset += HR_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSp02)
    {
        devStatusTmp.valueSp02 = gizY2X(Sp02_RATIO,  Sp02_ADDITION, dataPoints->valueSp02); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSp02,Sp02_LEN);
        byteFieldOffset += Sp02_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempcontrol)
    {
        devStatusTmp.valuetempcontrol = gizY2X(tempcontrol_RATIO,  tempcontrol_ADDITION, dataPoints->valuetempcontrol); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempcontrol,tempcontrol_LEN);
        byteFieldOffset += tempcontrol_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flaglatitude)
    {
        devStatusTmp.valuelatitude = exchangeBytes(gizY2XFloat(latitude_RATIO,  latitude_ADDITION, dataPoints->valuelatitude));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelatitude,latitude_LEN);
        byteFieldOffset += latitude_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglongitude)
    {
        devStatusTmp.valuelongitude = exchangeBytes(gizY2XFloat(longitude_RATIO,  longitude_ADDITION, dataPoints->valuelongitude));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelongitude,longitude_LEN);
        byteFieldOffset += longitude_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagppm)
    {
        devStatusTmp.valueppm = exchangeWord(gizY2XFloat(ppm_RATIO,  ppm_ADDITION, dataPoints->valueppm)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueppm,ppm_LEN);
        byteFieldOffset += ppm_LEN;
    }
